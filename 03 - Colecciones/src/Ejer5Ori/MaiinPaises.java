...
public class MaiinPaises {
    ...
    public static void main(String[] args) {
        ...
        do {
            Paises estado = new Paises();
            Paises estado2 = new Paises();
            Paises estado3 = new Paises();
            Paises estado4 = new Paises();
            ...
            aux4 = sc.next();
            estado.setName(aux);
            estado2.setName(aux2);
            estado3.setName(aux3);
            estado4.setName(aux4);
            pais.add(estado);
            pais.add(estado2);
            pais.add(estado3);
            pais.add(estado4);

            System.out.println("Quiere guardar otro pais a la lista?: (Si) para continuar ");
        ...
        System.out.println("elimine un pais ");
        String pedido = sc.next();
        Eliminador(pais, pedido);
        mostrar(pais);// no borra el pais de la lista ERROR

    }

    public static void mostrar(TreeSet<Paises> pais) {
    ...
    public static void Eliminador(TreeSet<Paises> p, String pedido) {

        Iterator<Paises> iterador = p.iterator();

        boolean noesta = false;
// "HasNext" es un boleano encargado de iterar si es cierto que existe un elemento a continuacion
        while (iterador.hasNext()) {
            /*
            Oriana: como en el for tradicional, guarabamos la variable centinela   
            en una variable para poder usarla.
            en la posicion "iterador.next()" de la collecion.
            Podemos guardarla antes de que de otra vuelta mas.            
             */
            String chiquito = iterador.next().getName();

            if (chiquito.equals(pedido)) {
                iterador.remove();
